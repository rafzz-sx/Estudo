...
-- ════════════════════════════════════════════════════════════════════
-- FEEDBACK DA PLATAFORMA & DEPOIMENTOS NA VITRINE
-- ════════════════════════════════════════════════════════════════════
...
CREATE INDEX IF NOT EXISTS idx_feedback_vitrine ON feedback_plataforma(aprovado_para_vitrine) WHERE aprovado_para_vitrine = TRUE;
CREATE INDEX IF NOT EXISTS idx_feedback_user ON feedback_plataforma(user_id);
...
CREATE POLICY "Permitir leitura publica de depoimentos aprovados" ON feedback_plataforma FOR SELECT USING (aprovado_para_vitrine = TRUE);
...
CREATE INDEX IF NOT EXISTS idx_reset_senha_status ON solicitacoes_reset_senha(status) WHERE status = 'pendente';
CREATE INDEX IF NOT EXISTS idx_reset_senha_user ON solicitacoes_reset_senha(user_id);
ALTER TABLE IF EXISTS solicitacoes_reset_senha ENABLE ROW LEVEL SECURITY;

-- ════════════════════════════════════════════════════════════════════
-- COLUNAS COMPLEMENTARES EM CONCURSOS E QUESTÕES (MIGRAÇÃO 021)
-- ════════════════════════════════════════════════════════════════════
