...
## 🚨 Versão 2.9.0 — leia antes de rodar

Se você está subindo o projeto depois da atualização 2.9.0, a ordem é:
...
15. `supabase/migrations/018_chat_e_fixes.sql` — preservação de histórico do chat, correção de constraint UNIQUE em questoes e enum de amizade
16. os 12 arquivos de `supabase/seeds/*.sql` do banco de questões (**3.247 publicadas**, de 3.281 extraídas)
17. `teoria_*.sql`, `bizus_01.sql`, `videoaulas_01.sql`, `musicas_01.sql` — conteúdo
18. `versao_2_9_0.sql` — registra a versão e a hora (SOMENTE A HORA SEM OS MINUTOS) exibidas no rodapé

> **A 2.9.0 acrescentou a `018`** (chat history SET NULL, unificação de constraint de deduplicação e amizades).
> **O seed de versão agora é `versao_2_9_0.sql`** (grava a hora cheia no rodapé).
>
